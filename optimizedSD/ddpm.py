...
class UNet(DDPM):
    ...
    def q_sample(
        self, x_start, t, index, seed=None, use_original_steps=False, noise=None
    ):
        ...
        if noise is None:
            ...
            del tens
        sqrt_alpha = torch.full(
            (x_start.shape[0], 1, 1, 1), sqrt_alphas_cumprod[index], device=self.cdevice
        )
        sqrt_one_minus_alpha = torch.full(
            (x_start.shape[0], 1, 1, 1),
            sqrt_one_minus_alphas_cumprod[index],
            device=self.cdevice,
        )
        ...
        for i, step in enumerate(iterator):
            ...
            if mask is not None and x0 is not None:
                img_orig = self.q_sample(
                    x0, ts, seed, use_original_steps=ddim_use_original_steps
                )
                img = img_orig * (1.0 - mask) + mask * img
    ...
    def p_sample_plms(
        ...
        alphas_prev = (
            self.alphas_cumprod_prev if use_original_steps else self.ddim_alphas_prev
        )
    ...
    def decode(
        ...
        for i, step in enumerate(iterator):
            ...
            x_dec, noise = self.p_sample_ddim(
                x_dec,
                cond,
                ts,
                index=index,
            ...
            if mask is not None and img_original is not None:
                ...
                x_dec = img_orig * mask + (1.0 - mask) * x_dec
